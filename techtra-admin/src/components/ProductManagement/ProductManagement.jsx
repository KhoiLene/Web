...
export default function ProductManagement() {
  ...
  const [selectedGroup, setSelectedGroup] = useState(""); // filter theo nhóm
  ...
  // ─── Fetch nhóm sản phẩm (cho dropdown filter) ───────────────────────────────
    ...
    <div className="main-content">
      ...
      <div style={{ display: "flex", gap: "10px", margin: "16px 0", flexWrap: "wrap", alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, minWidth: "200px" }}>
          ...
          <input type="text" placeholder="Tìm kiếm sản phẩm..." value={searchText}
      ...
      {error && (
        <div style={{ padding: "12px 16px", background: "#fef2f2", border: "1px solid #fecaca", borderRadius: "8px", color: "#dc2626", fontSize: "14px", marginBottom: "16px" }}>
          ⚠️ {error} — <button onClick={fetchProducts} style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", textDecoration: "underline" }}>Thử lại</button>
        </div>
      )}
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
              <thead>
                <tr style={{ background: "#f9fafb", borderBottom: "1px solid #e5e7eb" }}>
                  <th style={{ padding: "12px 16px", width: "40px" }}><input type="checkbox" checked={allSelected} onChange={toggleAll} /></th>
              ...
              <tbody>
                {products.map((p) => (
                  <tr key={p.id} style={{ borderBottom: "1px solid #f3f4f6" }}
                    ...
                    <td style={{ padding: "12px 16px" }}><input type="checkbox" checked={selected.includes(p.id)} onChange={() => toggleOne(p.id)} /></td>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        ...
                        <div>
                          <div style={{ fontWeight: "600", color: "#111827", cursor: "pointer" }}
                            onClick={() => setEditingProduct(p)}>{p.name}</div>
                    ...
                    <td style={{ padding: "12px 16px", textAlign: "center" }}>
                      <button onClick={() => setEditingProduct(p)}
                        style={{ background: "none", border: "none", color: "#2563eb", cursor: "pointer", fontSize: "14px", marginRight: "8px" }} title="Sửa">✏️</button>
                      <button onClick={() => handleDelete(p.id, p.name)}
                        style={{ background: "none", border: "none", color: "#dc2626", cursor: "pointer", fontSize: "14px" }} title="Xóa">🗑</button>
                    </td>
                  </tr>
                ))}
              </tbody>

...
//   const [videoFile,      setVideoFile]      = useState(null);
//   const [videoPreview,   setVideoPreview]   = useState(initialData?.video_url || null);
//   const [pdfUrl,         setPdfUrl]         = useState(initialData?.content_file || null); // Trỏ đúng vào content_file
...
//         {/* Mô tả & Bảng hiển thị tệp tin */}
//         <div style={{ marginBottom: "20px" }}>
//           <label style={label}><span style={req}>*</span> Mô tả</label>
//           <textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Nhập thông tin chi tiết sản phẩm..."
//             style={{ ...input, minHeight: "140px", fontFamily: "inherit", resize: "vertical", marginBottom: "12px" }} />
          
//           {/* BẢNG TRỰC QUAN CHI TIẾT CÁC TÀI LIỆU KÈM THEO */}
//           {(pdfUrl || videoPreview) && (
//             <div style={{ border: "1px solid #e5e7eb", borderRadius: "8px", overflow: "hidden", marginTop: "12px", background: "white" }}>
//               <div style={{ background: "#f3f4f6", padding: "10px 14px", borderBottom: "1px solid #e5e7eb", fontSize: "13px", fontWeight: "600", color: "#374151" }}>
//                 📋 Danh mục tài liệu đính kèm kèm mô tả
//               </div>
...
// }
import React, { useState, useRef, useCallback } from "react";
import { uploadImage } from "../../supabase"; // Hàm upload file lên Supabase Storage
import { productGroupsApi } from "../../api";
...
// ─── J&T API CONFIG ────────────────────────────────────────────────────────────
const JT_CONFIG = {
  BASE_URL: "https://api.jtexpress.vn/api",
...
async function fetchJTFee({ weight, weightUnit, height, width, length, serviceCode }) {
  ...
  if (!res.ok) throw new Error(`J&T API lỗi: ${res.status}`);
  const data = await res.json();
  return data.fee || data.totalFee || data.amount;
}
...
export default function CreateProduct({ onBack, onSaved, initialData = null }) {
  ...
  const handleSave = async () => {
    ...
    const body = {
      ...
      sku:           sku || null,
      slug:          slug || toSlug(productName),
      description:   description || "", // Chuỗi HTML mô tả gồm cả chữ lẫn ảnh
      group_id:      category || null,
      cod_enabled:   codEnabled,
      shipping_type: shippingMethod,
      weight:        parseFloat(weight)  || 0,
    ...
    <div style={{ padding: "4px", backgroundColor: "#f9fafb", minHeight: "100vh" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          <button
            onClick={handleSave}
            disabled={uploadingMain || uploadingSubs.some(Boolean) || uploadingVideo || uploadingPdf}
            style={{ padding: "9px 22px", borderRadius: "8px", border: "none", background: (uploadingMain || uploadingSubs.some(Boolean) || uploadingVideo || uploadingPdf) ? "#93c5fd" : "#2563eb", color: "white", fontWeight: "600", cursor: (uploadingMain || uploadingSubs.some(Boolean) || uploadingVideo || uploadingPdf) ? "not-allowed" : "pointer", fontSize: "14px" }}
          >
            {(uploadingMain || uploadingSubs.some(Boolean) || uploadingVideo || uploadingPdf) ? "⌛ Đang upload..." : "Lưu"}
          </button>
        </div>
      </div>

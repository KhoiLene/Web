/**
 * CỎ MỀM CLONE - JAVASCRIPT LOGIC & API INTEGRATION HOOKS
 *
 * File này quản lý toàn bộ logic tương tác giao diện và tích hợp Supabase.
 * Dữ liệu trang chủ (banner, values, categories, flash sale, blog, ...) lấy từ
 * homepageApi (xem /src/api.js). Có thể tắt bật từng section qua bảng homepage_config.
 */
...
async function loadHomepageData() {
    ...
    // 6. Flash sale
    if (data.config.sections.flashSale && data.flashSale && data.flashSale.products && data.flashSale.products.length) {
...
function renderBanners(banners) {
    ...
    if (!container) return;
    if (!banners || banners.length === 0) return; // giữ skeleton
    container.innerHTML = '';
    ...
    banners.forEach((banner, index) => {
        ...
        const slideHTML = `
            <div class="slide ${activeClass}" data-slide-id="${banner.id}">
                <div class="slide-bg" style="background-image: linear-gradient(rgba(255,255,255,0.7), rgba(255,255,255,0.8)), url('${banner.image}')"></div>
                <div class="container">

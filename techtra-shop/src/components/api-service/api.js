...
// - Danh mục nổi bật ← TẤT CẢ product_groups (is_active = true), click vào
//                      sẽ chuyển sang trang danh sách SP theo group
...
export const homepageApi = {
  async getAll() {
    const [
      ...
      groupsSliderRes, groupsActiveRes, productsAllRes,
    ] = await Promise.all([
      ...
      // Danh mục nổi bật: tất cả product_groups đang active (click → trang nhóm)
      supabase.from("product_groups").select("*").eq("is_active", true).order("sort_order"),
      // Lấy TẤT CẢ sản phẩm — dùng chung cho Flash sale
      supabase
        .from("products")
        .select("id, name, slug, category, price, old_price, image_url, rating, reviews, is_new, percent_sold, flash_sale_discount, flash_sale_end_at, created_at")
    ...
    // Map product_groups → categories (danh mục nổi bật trên trang chủ)
    const categories = (safe(groupsActiveRes).data || []).map((g) => ({
      id: g.id,
      name: g.name,
      imageUrl: g.image_url || g.image || "",
      // Click → trang danh sách SP theo group (kèm tên nhóm)
    ...
    return {
      ...
      slides,
      values:   (safe(valuesRes).data || []).map((r) => ({ id: r.id, icon: r.icon, title: r.title, desc: r.description })),
      categories,
